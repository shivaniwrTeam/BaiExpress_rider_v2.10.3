...
class CashCollectionProvider extends ChangeNotifier {
  ...
  Future<void> getOrder(
    ...
    delay = Timer(const Duration(milliseconds: 300), () async {
      if (isNetworkAvail) {
        try {
          if (isLoadingmore) {
            isLoadingmore = false;
            ...
            if (CUR_USERID != null) {
              ...
              if (!error) {
                ...
                if (data.length != 0) {
                  ...
                  print("all items 1*$allitems");
                  items.addAll((data as List)
                      .map((data) => CashColl_Model.fromJson(data))
                      .toList());
                  allitems.addAll(items);
                  // print("all items 2*$allitems");
                  // print("cash list 1 *${cashList.length}");
                  ...
                  //   ).toList();
                  // }
                  // print("cash list 2 *${cashList.length}");
